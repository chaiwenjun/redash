...
export default function init(ngModule) {
  ...
  function loadData($http, $route, $q, Auth) {
    return session($http, $route, Auth).then(() => {
      ...
      const query = $http.get(`redash/api/queries/${queryId}`).then(response => response.data);
      const queryResult = $http.get(`redash/api/queries/${queryId}/results.json${location.search}`).then(response => response.data);
